...
def upload(kind):
    ...
    try:
        ...
        if kind in ("foto", "recibo"):
            if is_demo:
                receipt = demo_receipt(kind == "recibo")
            else:
                require_ai_consent()
            ...
            from app.services.pdf_parser import demo_statement, parse_pdf

            if is_demo:
                movements = demo_statement()
            else:
                require_ai_consent()
                upload = request.files.get("file")
                if not upload:
...
def require_ai_consent():
    if not g.user.get("consentimientos", {}).get("iaDatos"):
        raise ValueError(
            "Activa el consentimiento de IA en Perfil para analizar documentos reales, o usa el ejemplo demo."
        )
