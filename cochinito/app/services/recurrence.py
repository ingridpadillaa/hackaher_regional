...
def detect_recurrences(movements):
    ...
    for m in movements:
        if m["tipo"] == "gasto" and not m.get("privado"):
            groups[normalize(m.get("comercio") or m["descripcion"])].append(m)
    result = []
    for merchant, items in groups.items():
        ...
        if not previous["monto"] or abs(last["monto"] / previous["monto"] - 1) > 0.15:
            continue
        ...
        if cadence:
            ...
            while next_day < local_today():
                next_day += timedelta(days=30 if cadence == "mensual" else 60)
            result.append(
                dict(
                    nombre=last.get("comercio") or last["descripcion"],
                    monto=last["monto"],
                    previous=previous["monto"],
                    increased=last["monto"] > previous["monto"] * 1.05,
