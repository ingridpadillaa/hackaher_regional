...
def add_period(day, periodicity, direction=1):
    if periodicity != "mensual":
        return day + timedelta(days=direction * (7 if periodicity == "semanal" else 15))
...
def period(household, today):
    ...
    start = date.fromisoformat(
        household.get("ultimaFechaIngreso") or add_period(end, cadence, -1).isoformat()
    )
    while end <= today:
...
def daily_budget(household, movements, payments, today=None, seasonal_weekly=0):
    ...
    if household.get("ingresoVariable"):
        ...
        for _ in range(6):
            ...
            total = sum(
                m["monto"]
                for m in movements
                if m["tipo"] == "ingreso" and left.isoformat() <= m["fecha"] < right.isoformat()
            )
            if total:
    ...
    for payment in payments:
        if (
            not start.isoformat() <= payment["proximaFecha"] < end.isoformat()
            or (payment.get("id"), payment["proximaFecha"]) in paid_ids
            or payment.get("pagado")
        ):
            continue
...
def health_score(household, movements, payments, today=None):
    ...
    if not dates or (today - min(dates)).days < 14:
        return None
    ...
    monthly = max(
        1, sum(m["monto"] for m in movements if m["tipo"] == "gasto") / max(1, (today - min(dates)).days / 30)
    )
