...
class Repository:
    ...
    def list(self, collection):
        ...
        return [dict(json.loads(value), id=path[len(prefix):]) for path, value in rows if '/' not in path[len(prefix):]]
...
def visible_movements(household_id, uid):
    return [m for m in repo().list(household_path(household_id, 'movimientos'))
