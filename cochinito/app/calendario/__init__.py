...
from app.services.seasonal import upcoming,estimate
...
def index():
    ...
    events=upcoming(repo().list('eventosTemporada'),household,visible_movements(g.hogar_id,g.user['uid']))
    month=request.args.get('mes','')
    if month: events=[e for e in events if e['fechaInicio'][5:7]==month]
...
def detail(event_id):
    ...
    item=estimate(dict(event,id=event_id),repo().get(f'hogares/{g.hogar_id}'),visible_movements(g.hogar_id,g.user['uid']))
