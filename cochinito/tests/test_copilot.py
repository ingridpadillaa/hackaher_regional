...
def test_math_tools():
    ...
    assert execute('simular_compra_a_meses',dict(monto=1200,meses=12,tasa_anual=0),context)['mensualidad']==100
...
def test_grounded_copilot(logged):
    token=csrf(logged,'/')
    response=logged.post('/copiloto/chat',json={'message':'¿Me alcanza para el regreso a clases?'},headers={'X-CSRF-Token':token})
