...
def test_new_account_has_no_sample_data(client, app):
    response=client.post('/auth/demo',data={'name':'Cuenta de prueba','csrf_token':csrf(client),'seed':'1'})
...
def test_household_types():
    ...
    assert household_type([{'edad':35},{'edad':9,'estudiaActualmente':True}])=='familia_con_hijos_escolares'


def test_empty_income_onboarding(client, app):
    client.post('/auth/demo',data={'name':'Prueba','csrf_token':csrf(client)})
    result=client.post('/onboarding',data={'csrf_token':csrf(client,'/onboarding'),'privacy':'1','nombre':'Hogar prueba','estado':'Estado prueba','municipio':'Municipio prueba','member_0_nombre':'Prueba','member_0_edad':'25'})
