...
def update_products(repository,hid):
    ...
    for ticket in repository.list(base+'/tickets'):
        ...
        for item in ticket.get('productos',[]):
            ...
            product['compras'].append(dict(fecha=ticket['fecha'],cantidad=item['cantidad'],precio=item.get('precioUnitario',item.get('precio')),tienda=ticket['tienda']))
    for pid,product in products.items():
        ...
        if len(days)>=2:
            ...
            product.update(intervaloPromedioDias=interval,proximaCompraEstimada=(days[-1]+timedelta(days=interval)).isoformat())
...
def suggestions(products):
    return [p for p in products if p.get('proximaCompraEstimada') and p['proximaCompraEstimada']<=(local_today()+timedelta(days=3)).isoformat()]
