...
def monthly_income(member):
    return member.get("ingreso", 0) * {"semanal": 52 / 12, "quincenal": 2, "mensual": 1, "variable": 0.9}.get(
        member.get("periodicidadIngreso"), 1
    )
...
def parse_members(form, uid):
    ...
    for i in range(20):
        ...
        member = {
            key: form.get(prefix + key, "")[:100]
            for key in ("parentesco", "escolaridad", "nivelQueEstudia", "ocupacion", "tipoEmpleo")
        }
        member.update(
            nombre=name[:80],
            edad=age,
            estudiaActualmente=bool(form.get(prefix + "estudiaActualmente")),
            ingreso=income,
            periodicidadIngreso=cadence,
            proximaFechaIngreso=payday,
        )
...
def derive_household(home, members):
    ...
    home.update(
        tipoHogar=household_type(members),
        ingresoMensualTotal=round(sum(monthly_income(m) for m in members), 2),
        calendarioIngresos=earners,
    )
    ...
    next_day = min(
        [m["proximaFechaIngreso"] for m in earners] or [add_period(local_today(), "mensual").isoformat()]
    )
    home.update(
        proximaFechaIngreso=next_day,
        ultimaFechaIngreso=add_period(date.fromisoformat(next_day), "mensual", -1).isoformat(),
    )
    return home
...
def parse_payments(form):
    ...
    for i in range(20):
        ...
        if cadence not in (*CADENCES[:3], "bimestral") or kind not in (
            "servicio",
            "suscripcion",
            "renta_colegiatura",
            "credito",
        ...
        payment = dict(
            nombre=name[:100],
            monto=amount(form.get(pre + "monto")),
            proximaFecha=due.isoformat(),
            diaDePago=due.day,
            periodicidad=cadence,
            tipo=kind,
            categoria=form.get(pre + "categoria", "Servicios"),
            origen="personalizacion",
        )
        if kind == "tarjeta_credito":
            ...
            payment["datosTarjeta"] = dict(
                ...
                pagoMinimo=minimum,
                pagoSinIntereses=payment["monto"],
                fechaLimite=due.isoformat(),
            )
        if kind == "credito":
            payment["datosCredito"] = dict(
                saldo=amount(form.get(pre + "saldo")), pagosRestantes=int(form.get(pre + "restantes") or 0)
            )
