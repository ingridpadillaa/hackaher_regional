...
def predict(tickets, today=None):
    ...
    for ticket in tickets:
        for product in ticket["productos"]:
            purchases[product["nombreNormalizado"]].add(date.fromisoformat(ticket["fecha"]))
    result = []
...
def compare_prices(prices, products):
    latest = {}
    for price in prices:
        if price["producto"] not in products:
            continue
        key = (price["producto"], price["tienda"])
        if (
            key not in latest
            or price["fecha"] > latest[key]["fecha"]
            or (price["fecha"] == latest[key]["fecha"] and price["fuente"] == "ticket")
        ):
            latest[key] = price
    comparisons = []
    for product in products:
        offers = sorted(
            [p for (name, store), p in latest.items() if name == product], key=lambda p: p["precio"]
        )
    ...
    for store in stores:
        ...
        totals.append(
            dict(
                store=store,
                total=round(sum(p["precio"] for p in offers), 2),
                count=len(offers),
                complete=len(offers) == len(products),
            )
        )
